...
void solve() {
    ...
    vector<par> arr;
    f(i,0,n){
        int custo, h; cin >> custo >> h;
        arr.pb({custo,h});
        
        soma += h;
    }
    
    vector<int> dp(soma+5,-1);
    vector<int> prx(soma+5,-1);
    
    dp[0] = 0;
    f(i,0,n){
        vector<int> mudou;
        auto [custo, alegria] = arr[i];
        
        f(i,0,dp.size()){
            if(dp[i]!=-1 && dp[i]>=custo){
                prx[i+alegria] = max(prx[i+alegria], dp[i]-custo);
