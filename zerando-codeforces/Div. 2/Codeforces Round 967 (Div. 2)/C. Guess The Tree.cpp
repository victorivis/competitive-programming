...
vector<ii> ans;
int rec(int l, int r){
    assert(st.count(l)!=0 and st.count(r)==0);

    while(true){
        int val = query(l,r);        
        if(val==l or val==r) break;
        if(st.count(val)) l = val;
        else r = val;
    };
...
void solve() {
    ...
    auto rng = views::iota(2ll,n+1);
    set<int> falta(all(rng));
    
    int lst=1;
    while(falta.empty()==false){
        int sai = rec(lst,*falta.begin());        
        falta.erase(sai);
        st.insert(sai);
    }
