// https://codeforces.com/contest/2249/problem/A
...
void solve() {
    ...
    f(m,1,n+1){
        int quant=0;
        f(i,0,n){
            int l=quant+1, r=m-quant;
            bool cond1 = (a[i].fi<=quant+1 and quant+1<=a[i].se);
            bool cond2 = (b[i].fi<=m-quant and m-quant<=b[i].se);
            quant = min(m,quant+(cond1==false and cond2==false));
        }
